...
## Where Ascend's reports module stands today
...
| Report scheduling / automated delivery | **Out of scope** — needs a job scheduler + email delivery, neither of which exist; Ascend is serverless (Vercel functions), so "scheduled reports" means a new cron/worker surface. Revisit if/when `webhooks` gains a notification channel for `ACCOUNTING_GAPS.md` BE-14 (AR dunning) — that's a smaller, more concrete first step toward "the system proactively tells you things." |
