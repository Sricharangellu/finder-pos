...
## Where Ascend's ecommerce module stands today
...
| Storefront / customer-facing site, B2B portal, product reviews | **Out of scope** — Ascend is the back-office; a storefront is a separate frontend product. If a tenant needs one, it consumes Ascend's existing `/api/v1/catalog` + `ecommerce` flag as a read API — no new backend work implied. |
