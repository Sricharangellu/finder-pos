...
## Where Ascend's discounts module stands today
...
**outdated for Ascend** — those rule types already exist in the schema and
...
| Discount rule builder UI (create/edit `simple`/`volume`/`bxgy` rules, tier restriction, coupon codes) | **Worth building** — `/discounts` page likely only lists rules today; an editor exposing the rule types Ascend already supports is the highest-value frontend gap here. |
