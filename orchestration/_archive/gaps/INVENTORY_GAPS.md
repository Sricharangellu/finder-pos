...
(Inventory) gaps worth building for Ascend, and turns it into roadmap items.

Updated: 2026-06-15.

## Where Ascend's inventory stands today
...
| Stock transfer between locations / multi-warehouse | **Defer.** Ascend is single-warehouse per tenant today (`outlets` exist but inter-store stock movement isn't modeled). Revisit alongside BE-4 (multi-store filter) once tenants actually run >1 warehouse. |
...
| Demand forecasting, FIFO/LIFO/WAC valuation method selection | **Out of scope.** Ascend uses FEFO for expiry-tracked SKUs and a single cost basis; a configurable valuation method is ERP-generic complexity Ascend's users won't ask for. |
...
| Consignment inventory, cross-docking | **Out of scope** — not relevant to Ascend's wholesale/retail customer base. |
