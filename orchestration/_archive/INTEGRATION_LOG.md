...
## 2026-06-30 — Frontend cycle: FE-51

- **Shipped:** Receiving → Purchasing pipeline. (1) `/inventory/receive-stock/page.tsx` rebuilt as standalone page: barcode scan with auto-PO selection and line highlight, cases × units/case live total, expiry/lot per line, sort toggle (insertion order vs A-Z), document upload, submit to `POST /api/v1/purchasing/orders/:id/receive`. (2) `/purchasing/[id]/page.tsx` rebuilt as 4-tab interface: Lines tab (product name, SKU, margin %, cost vs last cost with delta arrow, sell price, lot/expiry, price history panel); Receive tab (cases × units form + doc attach + per-line status summary); Billing tab (landed costs editor, price adjustments +/- with optional per-line assignment, net billing total); Credits tab (vendor credits filtered by poId, create chargeback/credit memo with source picker: expired lots / damaged / manual, void action, open total).
- **Consumes:** `GET/POST /api/v1/purchasing/orders/:id`, `GET /api/v1/purchasing/orders/:id/price-history`, `GET/POST /api/v1/purchasing/orders/:id/documents`, `POST /api/v1/purchasing/orders/:id/landed-costs`, `GET/POST /api/v1/purchasing/orders/:id/billing-adj`, `GET/POST /api/v1/purchasing/vendor-credits` (with poId filter), `POST /api/v1/purchasing/vendor-credits/:id/void`, `GET /api/v1/inventory/expired` — all mocked via MSW IIFE block in `web/mocks/mockHandlers.ts`.
- **Verified:** `npx tsc --noEmit` in `web/` — 0 errors.
- **Contract changes:** none; all new endpoints are mock-only until backend implements them. Backend lane should add: price-history endpoint, documents CRUD on POs, landed-costs mutation, billing-adj CRUD, vendor-credits poId filter, void action.

## 2026-06-30 — Frontend cycle: FE-52

- **Shipped:** Reorder suggestions enhanced with vendor history and product sort view. (a) "By vendor" mode: vendor group header now shows last ordered date and PO count inline; "View history" toggle expands a panel listing last 2-3 POs from that vendor (PO#, date, total, item count, status). Item rows gain Last cost and Last ordered columns; out-of-stock items show red dot indicator. (b) "By product" mode (new): pill toggle switches to a flat A-Z table of all items across all vendors, with a Vendor column — good for "what do I need to order today" scanning without caring about vendor grouping.
- **Consumes:** `GET /api/v1/inventory/reorder-suggestions` (extended: now returns `last_unit_cost_cents`, `last_ordered_at`, `last_ordered_qty` per item); `GET /api/v1/purchasing/vendor-history` (new mock: returns per-vendor PO summary lists, keyed by vendor ID).
- **Verified:** `npx tsc --noEmit` in `web/` — 0 errors.
- **Contract changes:** backend should implement `GET /api/v1/purchasing/vendor-history?vendorId=:id` returning `{ items: [{ po_id, po_number, created_at, total_cost_cents, item_count, status }] }`. Also extend reorder-suggestions response to include last purchase data per item (derivable from `product_costs` join on PO lines).
