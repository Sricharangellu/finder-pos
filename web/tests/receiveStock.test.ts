...
function entry(over: Partial<ReceiveEntry>): ReceiveEntry {
  return { lineId: "l1", cases: "1", unitsPerCase: "1", totalQty: 1, expiryDate: "", lotCode: "", ...over };
}

describe("buildReceiveLines", () => {
  it("carries desk-entered expiry (as epoch ms) and lot code through to the payload", () => {
    const lines = buildReceiveLines([
      entry({ lineId: "l1", totalQty: 12, expiryDate: "2027-06-30", lotCode: "LOT-9" }),
    ]);
    expect(lines).toEqual([
      { lineId: "l1", qty: 12, expiryDate: new Date("2027-06-30").getTime(), lotCode: "LOT-9" },
    ]);
  });

  it("omits expiry and lot when the operator left them blank (no null noise)", () => {
    const lines = buildReceiveLines([entry({ lineId: "l2", totalQty: 5, expiryDate: "", lotCode: "   " })]);
    expect(lines).toEqual([{ lineId: "l2", qty: 5 }]);
    expect(lines[0]).not.toHaveProperty("expiryDate");
    expect(lines[0]).not.toHaveProperty("lotCode");
  });

  it("drops lines with zero quantity", () => {
    const lines = buildReceiveLines([
      entry({ lineId: "l1", totalQty: 0, lotCode: "X" }),
